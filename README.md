...
.NET Core Docker images can use multi-stage build. This feature allows multiple FROM lines to be used in one Dockerfile. Using this feature, you can build a .NET Core app in an SDK image (AKA 'build image') and then copy the published app into a lighter weight runtime image all in the same Dockerfile.
