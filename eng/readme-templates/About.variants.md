...
Apps that require globalization support can use the `extra` image variant of the [dotnet/runtime-deps]({{InsertTemplate("Url.md", [ "readme-host": ARGS["readme-host"], "repo": "dotnet/runtime-deps" ])}}) images. {{if or(repo = "runtime-deps", repo = "product-family"):
...
Extra images contain everything that the default images do, plus `icu` and `tzdata`.}}{{if or(repo = "aspnet", repo = "product-family"):
...
However, this means that apps run on the ASP.NET Composite runtime cannot use handpicked custom versions of .NET or ASP.NET assemblies that are built into the image.}}{{if or(repo = "runtime-deps", repo = "sdk", repo = "product-family"):
