...
try {
    ...
    if ($activeOS -eq "linux") {
        # On Linux a ImageBuilder is run within a Docker image.  The repo is copied into a Docker volume in order to support
        # using remote Docker servers (local volumes not possible).
        ...
        # On Windows, ImageBuilder is run locally.
