...
The `MultiProjectApp` sample is intended to act as a [very simple](MultiProjectApp/Program.cs) "complex application". It is composed of multiple projects, including a test project. It is used to demonstrate various workflows.

Simpler workflows are provided at [.NET Docker samples](../README.md).

The instructions assume that you have cloned this repo, have [Docker](https://www.docker.com/products/docker) installed, and have a command prompt open within the `samples/MultiProjectApp` directory within the repo.
...
This is the approach used with [MultiProjectApp](.). The [Dockerfile](./Dockerfile) for the sample is at the same location as the `.slnx` file, and all projects are available when that same location is used as the build context. There are other options, but this approach is the easiest.

You can build and run MultiProjectApp using the following commands:
...
[Running Tests with Docker](../run-tests-in-sdk-container.md) is one example that covers running this sample's tests using the separate executable test stage in the [`MultiProjectApp` Dockerfile](./Dockerfile).
