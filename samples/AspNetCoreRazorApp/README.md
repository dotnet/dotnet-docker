...
This sample demonstrates how to build container images for ASP.NET Core web apps. It is based on the `dotnet new webapp` template. See [.NET Docker Samples](../README.md) for more samples.
...
CONTAINER ID   IMAGE       COMMAND         CREATED         STATUS         PORTS                                         NAMES
d021ffa2e15f   aspnetapp   "./AspNetCoreRazorApp"   2 minutes ago   Up 2 minutes   0.0.0.0:8000->8080/tcp, [::]:8000->8080/tcp   reverent_aryabhata
```
...
```console
docker build --pull -t aspnetapp 'https://github.com/dotnet/dotnet-docker.git#:samples/AspNetCoreRazorApp'
```
...
```console
docker build --pull -t dotnetapp -f Dockerfile.chiseled 'https://github.com/dotnet/dotnet-docker.git#:samples/ConsoleApp'
```
