...
> Note: The password must match the password used for the certificate. The `--user ContainerAdministrator` flag has been added as a workaround since there's a potential and intermittent race condition at the container/app startup that throws a `WindowsCryptographicException`. This is a known bug and it has been [reported here](https://github.com/dotnet/runtime/issues/70386)
